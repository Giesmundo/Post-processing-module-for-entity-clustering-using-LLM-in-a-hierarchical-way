...
OUTPUT_PATH = r"D:\Tesi PY\output\th6_Clusters_Puliti.json"
GROUND_TRUTH = r"D:\Tesi PY\Ground_truth\C_GROUND_TRUTH_O.json"
ERRORS_PATH = r"D:\Tesi PY\errors\errori_clean.json"
GENAI_MODELS = ["gemini-3.5-flash", "gemini-3.1-flash-lite", "gemini-3-flash-preview"]
...
def process_clusters_parallel(Sep_C: dict[int, list], max_workers: int) -> dict[str, list]:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for batch, cluster_list in Sep_C.items():
            future_obj = executor.submit(O_call_llm, cluster_list, prompt)
...
def run(input_path: str, context: bool, NER_type: bool, batch_length: int, output_path: str = OUTPUT_PATH):
    ...
    for cluster in Cleaned_documents:
        if cluster is None:
            Cleaned_documents.remove(cluster)
        else:
